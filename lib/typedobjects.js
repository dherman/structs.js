...
function valueType(size, View, name) {
  var conversionView = new View(1);
  var result = {}
  result._size = size;
  result._View = function(s, byteOffset, length) {
    return new View(s.arrayBuffer, byteOffset, length);
  }
  result._View.BYTES_PER_ELEMENT = View.BYTES_PER_ELEMENT;
...
function StructType(typeObj) {
  ...
  result._View.BYTES_PER_ELEMENT = size;
  ...
  var result = function(o, byteOffset, o1) {
    ...
    if (fixedLength) {
      ...
      if (o instanceof Storage) {
        ...
        storage = o;
        view = elementType._View(storage, byteOffset, viewLength);
      } else {
        var storage = new Storage(new ArrayBuffer(viewLength * elementType._size), opaque);
        view = elementType._View(storage);
      }
    } else {
      viewLength = o;
      var storage = new Storage(new ArrayBuffer(viewLength * elementType._size), opaque);
      view = elementType._View(storage, 0, viewLength);
    }
