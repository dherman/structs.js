...
function StructView(S) {
  ...
  result.prototype.setItem = function(index, v) {
    print("Setting item at " + index + " to " + JSON.stringify(v));
...
function StructType(typeObj) {
  ...
  //print(JSON.stringify(internals));

  var result = function(o, offset, o1) {
    var views = new Array(viewTypes.length);

    print("o = " + o + " offset = " + offset + " o1 = " + JSON.stringify(o1));
    ...
    if (!(o instanceof ArrayBuffer) && o) {
      for (var i = 0; i < props.length; i++) {
        pName = props[i];
        print("Assigning to " + pName + ": " + o[pName]);
        this[pName] = o[pName]; 
      }
    } else if (o1) {
      for (var i = 0; i < props.length; i++) {
        pName = props[i];
        print("Assigning to " + pName + ": " + o1[pName]);
        this[pName] = o1[pName]; 
      }
    }
  }
...
function ArrayType(elementType, length) {
  ...
  var result = function(o, byteOffset, o1) {
    ...
    if (o instanceof ArrayBuffer) {
      ...
      view = new elementType._View(viewLength);
    }
    ...
    if (fixedLength) {
      for (var i = 0; i < viewLength; i++) {
        (function() {
          print("i = " + i + ", elementType._size = " + elementType._size);
          ...
          Object.defineProperty(
            ...
            { configurable:false,
              ...
              set: function(v) {
                print("Setting elem at offset " + off);
      ...
      if (!(o instanceof ArrayBuffer) && o) {
        print("Intitializing with " + JSON.stringify(o));
        for (i = 0; i < Math.min(viewLength, o.length); i++) {
          self[i] = o[i];
          print("self[" + i + "] = " + JSON.stringify(self[i]));
        }
      }
    }
  }
